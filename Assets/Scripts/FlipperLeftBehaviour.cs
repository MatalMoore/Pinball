...
public class FlipperLeftBehaviour : MonoBehaviour {
    HingeJoint2D hinge;
    JointMotor2D motor;
    ...
    void Update() {
        ...
        if (hinge.limitState == JointLimitState2D.LowerLimit && state != Values.FlipperState.goingDown) {
            state = Values.FlipperState.up;
        }
        if (hinge.limitState == JointLimitState2D.UpperLimit && state != Values.FlipperState.goingUp) {
